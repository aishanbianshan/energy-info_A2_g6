...
SVR performs the best out of the four models with an RMSE of 0.214, as seen in Table \ref{tab:q1-RMSE-comparison} 
showing the RMSE value of each of the four models. The ANN is second best with an RMSE of 0.215 and LR/KNN has the same RMSE of 0.217.
%  add some words about why svr performs the best and why other three are not suitable
\begin{table}[H]
    \centering
    \caption{\label{tab:q1-RMSE-comparison} Table of RMSE for the Four Methods}
...
The prediction using Multiple Linear Regression is slightly more accurate than the Linear Model, especially during days of higher variance. This is also reflected in the RMSE being slightly lower for MLR. MLR is taking more parameters into the model, wh
...
For the recurrent neural network we need to preprocess the data by making sequences. We divide the data into sequences based on a window size so that we can feed in the window size to the network and it will predict the next value based on that window.
